function M=jsonDecodeLegacy(J,tag)
%JSONDECODE converts a javscript data object (JSON) into a Matlab structure
...
if exist(J)==2 % if J is a filename
    ...
    elseif J1(1)=='"' %literal string
        ...
        if length(j)==length(J) %it is a number
            ...
            M=jsonDecodeLegacy(['"',J,'"']); % not a number after all
        end
    end
end
...
n=size(indOC,1);
for i=n:-1:1
    tag{i}=jsonDecodeLegacy(x(indOC(i,1):indOC(i,2)));
...
for i=n+m:-1:n+1
    j=i-n;
    tag{i}=jsonDecodeLegacy(x(indOC(j,1):indOC(j,2)),tag);
...
for i=1:n
    ...
    if regexp(t{1}{2},'tag{\d+}')
        ...
        y.(t{1}{1})=jsonDecodeLegacy(t{1}{2});
    end
    %y.(t{1}{1})=jsonDecode(t{1}{2});
end
